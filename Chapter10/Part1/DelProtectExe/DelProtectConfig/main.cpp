...
int wmain(int argc, wchar_t* argv[]) {
	if (argc < 3) {
		std::cout << "Usage: DelProtectConfig <option> [exename]\n";
	...
	if (_wcsicmp(argv[1], L"add") == 0) {
		success = DeviceIoControl(hFile, IOCTL_DELPROTECT_ADD_EXE, argv[2], (wcslen(argv[2]) + 1) * sizeof(WCHAR), nullptr, 0, &returnd, nullptr);
	}
	else if (_wcsicmp(argv[1], L"remove") == 0) {
		success = DeviceIoControl(hFile, IOCTL_DELPROTECT_REMOVE_EXE, argv[2], (wcslen(argv[2]) + 1) * sizeof(WCHAR), nullptr, 0, &returnd, nullptr);
	}
