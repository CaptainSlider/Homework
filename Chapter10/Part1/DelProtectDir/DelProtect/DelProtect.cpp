...
int FindDir(PUNICODE_STRING DirName, bool DosName);
...
bool IsDeleteAllowed(PFLT_CALLBACK_DATA Data) {
    ...
    auto status = nameInfo.Parce();
    if (!NT_SUCCESS(status)) {
        return allow;
    }

    UNICODE_STRING path;
    path.Length = nameInfo->Volume.Length + nameInfo->Share.Length + nameInfo->ParentDir.Length;
    path.Buffer = nameInfo->Volume.Buffer;

    AutoLock <FastMutex> lock(g_Globals.DirectoryLock);
...
NTSTATUS DelProtectDeviceControl(PDEVICE_OBJECT, PIRP Irp) {
    ...
    case IOCTL_DELPROTECT_ADD_DIR: {
        ...
        auto bufferLenght = stack->Parameters.DeviceIoControl.InputBufferLength;
        if (bufferLenght > 1000) {
            status = STATUS_INVALID_PARAMETER;
            break;
        }

        name[bufferLenght / sizeof(WCHAR) - 1] = L'\0';
        ...
        if (g_Globals.DirectoryCount == MaxDirectories) {
        ...
        for (int i = 0; i < MaxDirectories; ++i) {
            if (g_Globals.DirectoryNames[i].DosPath.Buffer == nullptr) {
                ...
                status = ConvertDosNameToNtName(buffer, &g_Globals.DirectoryNames[i].NtPath);
                if (!NT_SUCCESS(status)) {
                    ExFreePool(buffer);
                    break;
                }

                RtlInitUnicodeString(&g_Globals.DirectoryNames[i].DosPath, buffer);
                ++g_Globals.DirectoryCount;
                break;
            }
        }
        break;
    ...
    case IOCTL_DELPROTECT_REMOVE_DIR: {
        ...
        if (auto i = FindDir(&dosName, TRUE) >= 0) {
            g_Globals.DirectoryNames[i].Free();
            --g_Globals.DirectoryCount;
        }
        else {
...
int FindDir(PUNICODE_STRING DirName, bool DosName) {
    if (g_Globals.DirectoryCount == 0) {
        return -1;
    }

    for (int i = 0; i < g_Globals.DirectoryCount; i++) {
        auto& dir = DosName ? g_Globals.DirectoryNames[i].DosPath : g_Globals.DirectoryNames[i].NtPath;
        if (RtlEqualUnicodeString(&dir, DirName, TRUE)) {
            return i;
        }
    }
    return -1;
...
NTSTATUS ConvertDosNameToNtName(PCWSTR DosName, PUNICODE_STRING NtName) {
    ...
    const int symLinkLen = 20;
    WCHAR symLink[symLinkLen] = L"\\??\\";
    wcsncat_s(symLink, symLinkLen, DosName, 2); // \??\X:

    UNICODE_STRING symLinkFull;
    OBJECT_ATTRIBUTES symLinkAttr;
    RtlInitUnicodeString(&symLinkFull, symLink);
    InitializeObjectAttributes(&symLinkAttr, &symLinkFull, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, nullptr, nullptr);
    auto status = STATUS_SUCCESS;
    HANDLE hLink;
    do
    {
        status = ZwOpenSymbolicLinkObject(&hLink, GENERIC_READ, &symLinkAttr);
        if (!NT_SUCCESS(status)) {
            break;
        }
